		...
		switch(cmd) {
			case "list":
				function callback(addedMembersString) {
					const addedMembersEmbed = new MessageEmbed()
				...
				if(memberPrivChannel.permissionOverwrites.cache.size == 2) {
					...
					memberPrivChannel.permissionOverwrites.cache.forEach(addedMember => {
						if(addedMember.type == 'role') return;
			...
			case "addtemp":
				...
				return message.reply({ //repliug on command success
					content: `:white_check_mark:  Dodano tymczasowo **${memberToTempAdd.user.tag}** do prywatnego kanału`,
					allowedMentions: {
						repliedUser: false
					}
				});
