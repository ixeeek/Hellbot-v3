...
module.exports = {
	...
	async execute(message, args){
		...
		//code
		if(!message.channel.nsfw) return noPingReply({message: message, content: `Tej komendy możesz użyć tylko na kanale NSFW!`});
		if(!type) return noPingReply({message: message, content: `Nie znaleziono! Użyj: \`${list}\``});
		...
		switch(type) {
			...
			case "solo":
				...
				break;
			default:
				return noPingReply({message: message, content: `Nie znaleziono! Użyj: \`${list}\``});
				break;
		};
